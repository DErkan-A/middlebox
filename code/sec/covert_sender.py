...
def send_chunk(dst_ip, chunk_data, seq_num):
    ...
    for i in range(chunk_size):  # Send chunk_size data packets
        if i < len(chunk_data):
            packets.append((ord(chunk_data[i]), chunk_data[i]))
        else:  # Pad if message is short
...
def wait_for_response(dst_ip, timeout, iface):
    ...
    while time.time() - start_time < timeout:
        packets = sniff(iface=iface, filter=f"ip and src host {dst_ip} and ip proto 6",
                       timeout=0.1, count=1)
        if packets:
            ...
            if proto_val == 6:
                ...
                return True, rtt
    print(f"Timeout after {timeout:.3f}s")
